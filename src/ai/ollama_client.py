...
def generate_response(prompt: str, model: str = config.OLLAMA_MODEL) -> str | None:
    """
    ...
    Args:
        ...
        model (str):
            Which Ollama model to use. Defaults to config.OLLAMA_MODEL.
    ...
    try:
        ...
        logger.warning(
            "Ollama request timed out after %d seconds.", config.OLLAMA_TIMEOUT_SECONDS
        )
        return None
